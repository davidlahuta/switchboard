...
import { HOME_CLAUDE_DIR } from '../config.ts';
...
export class Updater {
  ...
  private async readVersion(): Promise<string | null> {
    ...
    try {
      ...
      const { stdout } = await run(cmd.file, cmd.args, { timeout: 30_000, windowsHide: true });
  ...
  async check(manual: boolean): Promise<UpdateStatus> {
    ...
    try {
      ...
      const { stdout, stderr } = await run(cmd.file, cmd.args, { timeout: UPDATE_TIMEOUT_MS, windowsHide: true });
